...
internal class MainRepository(context: Context) {
    ...
    var selectedGender: UserGender
        get() {
            val value = sharedPreferences.getString("gender", UserGender.MALE.name)
            return UserGender.valueOf(value ?: UserGender.MALE.name)
        }
        set(value) = sharedPreferences.edit { putString("gender", value.name) }

    var nameText: String
        get() = sharedPreferences.getString("name", "PRAI") ?: "PRAI"
